...
public class TareaFacadeImpl extends BaseFacadeImpl implements TareaFacade {
		...
		try {
			...
			if (!tareas.isEmpty()) {
				for (Tarea tarea : tareas) {
					...
					if (tarea.getTarea().equals(EnumeracionBitacora.CONFIRMAR_NOTIFICACION_REQ.getClave())
							|| tarea.getTarea().equals(EnumeracionBitacora.ATENDER_REQ_AUTORIDAD.getClave())) {
						Requerimiento reqAutoridad = requerimientoServices
								.obtenerRequerimientoAutoridadInterna(tarea.getNumeroAsunto());
	...
	public boolean tieneDocumentosAnexados(String idSolicitud) {
		return tramiteServices.tieneDocumentosAnexados(idSolicitud);
	}

	@Override
	public FirmaDTO obtieneFirma(DatosBandejaTareaDTO datoSelected) {
		Firma firma =firmarServices.obtenerFirmaSolicitud(datoSelected.getIdSolicitud().toString(), datoSelected.getRfcSolicitante());
		FirmaDTO firmaDTO = new FirmaDTO();
		firmaDTO.setCadenaOriginal(firma.getCadenaOriginal());
		firmaDTO.setFechaFirma(firma.getFechaFirma());
