...
public class SolicitudServiceImpl extends BaseSerializableBusinessServices implements SolicitudService {
	...
	/**
	 * Atributo generarNumeroAsuntoService tipo GenerarNumeroAsuntoService
	 */
	@Autowired
	...
	/**
	 * Atributo registroRecursoRevocacionDAO tipo RegistroRecursoRevocacionDAO
	 */
	@Autowired
	...
	public void iniciaTramite(MensajeBPM mensajeBPM, SolicitudDatosGenerales sol, String rfcResponsable) {
		...
		try {
			...
			if (mensajeBPM.getbDoc().contains("<root>")) {
				json = XML.toJSONObject(mensajeBPM.getbDoc().replace("<root>", "").replace("</root>", ""));
			} else {
				json = new JSONObject(mensajeBPM.getbDoc());
			}
			tramiteReprocesarServices.guardarTramiteAReprocesar(json, mensajeBPM.getBpdId(),
					mensajeBPM.getProcessAppId(), TipoErrorEnvioBPM.INEXISTENTE);
	...
	/**
	 * Metodo para generar la cadena original del registro de promocion RRL
	 * 
	 * @param idSolicitud
	 * @param fechaFirma
	 * @return cadena
	...
	/**
	 * Metodo para generar la cadena original de atencion de requerimiento
	 * 
	 * @param idSolicitud
	 * @param fechaFirma
	 * @return cadena
	...
	public mx.gob.sat.siat.juridica.bpm.dao.domain.Tramite generarMensajeInicio(SolicitudDatosGenerales sol,
		...
		mx.gob.sat.siat.juridica.bpm.dao.domain.Tramite inicioTramite = new mx.gob.sat.siat.juridica.bpm.dao.domain.Tramite();
	...
	public Tramite generarTramite(long idSolicitud, Firma firmas, Object ceritifcadoUtilizado, String prefijoTramite,
		...
		String secuencia = generarNumeroAsuntoService.obtenerSecuencia(TipoSecuencia.SOLICITUD.getClave(), anio,
				idTipoTramiteLocal);
	...
	public void sellaDocumentos(Long idSolicitud) {
		...
		List<DocumentoSolicitud> docs = registroRecursoRevocacionDAO
				.obtenerDocumentoSolicitudAnexado(idSolicitud.toString());
		Firma selloFirma;
	...
	public void firmarDocumentos(Long idSolicitud, Firma firma, boolean complementario) {
		...
		AsignacionTramite abogado = asignacionTramiteServices.buscarAsignacionesRolAsunto(tramite.getNumeroAsunto(),
				RolesConstantes.ROL_INTERNO_EMPLEADO_ABOGADO);
		if (abogado != null) {
			enviarCorreoService.enviarCorreoDocAdicionalesAbogado(tramite.getNumeroAsunto(),
					abogado.getPersona().getRfc());
		}
	...
	/**
	 * Genera una Firma con el sello, cadena y certificado de la selladora del SIAT
	 * 
	 * @param numAsunto
	 * @param idSolicitud
	 * @param fechaFirma
	 * @return Una firma generada con los datos devueltos por el SIAT, con la cadena
	 *         del registro de promocion cuando firma el SIAT, o una Firma con la
	 *         cadena sin sellio ni serie de certificado si ocurre una excepcion
	 */
	@Override
	...
	/**
	 * Genera una Firma con el sello, cadena y certificado de la selladora del SIAT
	 * con los datos del registro de promocioni RRL
	 * 
	 * @param numAsunto
	 * @param idSolicitud
	 * @param fechaFirma
	...
	public void modificarSolicitudRRL(SolicitudDatosGenerales solicitud) {
		SolicitudRecursoRevocacion solicitudBase = (SolicitudRecursoRevocacion) solicitudDAO
				.obtenerSolicitudDatosGeneralesPorId(solicitud.getIdSolicitud());
		solicitudBase
				.setUnidadAdministrativaRepresentacionFederal(solicitud.getUnidadAdministrativaRepresentacionFederal());
	...
	public List<Object> firmarSolicitud(long idSolicitud, Firma firma, String usuario, String rfcContribuyente,
		...
		getLogger().debug("Profiling inicio service obtenerTramitePorIdSolicitud {}",
		...
		getLogger().debug("Profiling fin service obtenerTramitePorIdSolicitud {}",
				new SimpleDateFormat(FORMATO_FECHA).format(new Date()));
		if (tramite == null) {

			getLogger().debug("Profiling inicio service generarTramite {}",
					new SimpleDateFormat(FORMATO_FECHA).format(new Date()));
			tramite = solicitudService.generarTramite(idSolicitud, firma, ceritifcadoUtilizado,
					DiscriminadorConstants.PREFIJO_RRL, null);
			getLogger().debug("Profiling fin service generarTramite {}",
					new SimpleDateFormat(FORMATO_FECHA).format(new Date()));

			getLogger().debug("Profiling inicio service guardarTramite {}",
					new SimpleDateFormat(FORMATO_FECHA).format(new Date()));
			solicitudService.guardarTramite(tramite);
			getLogger().debug("Profiling fin service guardarTramite {}",
					new SimpleDateFormat(FORMATO_FECHA).format(new Date()));
		}

		SolicitudDatosGenerales solDat = (SolicitudDatosGenerales) tramite.getSolicitud();

		getLogger().debug("Profiling inicio service obtenerUnidadPorId {}",
				new SimpleDateFormat(FORMATO_FECHA).format(new Date()));
		UnidadAdministrativa unidadLocal = unidadAdministrativaServices
				.obtenerUnidadPorId(tramite.getSolicitud().getUnidadAdministrativaRepresentacionFederal().getClave());
		getLogger().debug("Profiling fin service obtenerUnidadPorId {}",
				new SimpleDateFormat(FORMATO_FECHA).format(new Date()));
		...
		if (unidadLocal.isBlnLocal()) {
			...
			tramite.setSolicitud(
					solicitudService.actualizarUnidadAdminBalanceo(tramite.getSolicitud(), unidadAdmin.getClave()));

			getLogger().debug("Profiling inicio service obtenerAutorizadorResponsableUnidadLocalRecaudadora {}",
					new SimpleDateFormat(FORMATO_FECHA).format(new Date()));
			ResultadoAdminResponsable resultado1 = balanceadorServices
					.obtenerAutorizadorResponsableUnidadLocalRecaudadora(
							(SolicitudDatosGenerales) tramite.getSolicitud());
			getLogger().debug("Profiling fin service obtenerAutorizadorResponsableUnidadLocalRecaudadora {}",
					new SimpleDateFormat(FORMATO_FECHA).format(new Date()));
			...
			getLogger().debug("Profiling inicio service obtenerAutorizadorResponsable {}",
					new SimpleDateFormat(FORMATO_FECHA).format(new Date()));
			ResultadoAdminResponsable resultado = balanceadorServices.obtenerAutorizadorResponsable(solDat);
			getLogger().debug("Profiling fin service obtenerAutorizadorResponsable {}",
					new SimpleDateFormat(FORMATO_FECHA).format(new Date()));
		...
		mx.gob.sat.siat.juridica.bpm.dao.domain.Tramite inicioTramite = solicitudService.generarMensajeInicio(solDat,
				tramite, usuario, rfcContribuyente, administradorResponsable);
